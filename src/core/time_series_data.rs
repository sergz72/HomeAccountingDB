...
pub trait Loader<K> {
    fn load(&mut self, file_name: String, data_source: &Box<dyn DataSource<K>>) -> Result<(), Error>;
}
...
impl<T: Loader<K>, K> TimeSeriesData<T, K> {
    pub fn load(data_folder_path: String, source: Box<dyn DataSource<K>>,
                date_parser: fn(&FileInfo) -> Result<u64, Error>,
                creator: fn() -> T) -> Result<TimeSeriesData<T, K>, Error> {
        let file_list = get_file_list(data_folder_path)?;
        let mut map = BTreeMap::new();
        for file in file_list {
            let date = date_parser(&file)?;
            map.entry(date).or_insert(creator()).load(file.name, &source)?;
        }
